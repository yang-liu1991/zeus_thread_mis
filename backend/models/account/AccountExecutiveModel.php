...
class AccountExecutiveModel extends AacBaseModel
{
	...
	/**
	 *	根据实体id、推荐人、及提交开户的时间获取所有开户信息，
	 *	这样可以保证每个agency都提交自己的信息，避免agency之间的冲突
	 *	@params int	entity_id
	 *	@params	str	referral
	 *	@params	str	created_at
	 *	@return array
	 */
	private function getAdAccountsInfo($entityId, $referral, $createdAt)
	{
		try {
			$sql = sprintf('select fbaccount_name, timezone_id from th_account_info where 
				entity_id = %d and referral = "%s" and created_at = %s', $entityId, $referral, $createdAt);
			$connections	= Yii::$app->db;
			$command		= $connections->createCommand($sql);
			...
			Yii::error(sprintf("[getAdAccountsInfo] Exception, entityId:%d, referral:%s, createdAt:%s, reason:%s", 
				$entityId, $referral, $createdAt, $message->getMessage()));
			return false;
		}
	}
	...
	public function initAttributes($data)
	{
		try {
			...
			$adAccountsInfo	= $this->getAdAccountsInfo($data['entity_id'], $data['referral'], $data['created_at']);
			$adAccountInfoList	= [];
		...
		try {
			...
			if($response)
			{
				...
				if(property_exists($result, 'ad_account_creation_request_id'))
				{
					...
					$this->entity_id, $this->referral, $this->account_createdtime
					)) return true;
				}
				return $response;
			}
	 ...
	 *	@return bool
	 */
	private function accountUpdateByEntityId($attributes, $entity_id, $referral, $created_at)
	{
		$setValues = '';
		...
		$sql = sprintf('update th_account_info set %s where entity_id = %d and referral = "%s" and created_at = %s',
