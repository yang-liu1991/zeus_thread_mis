$(document).ready(function() {
	...
	function render(d,data,i) {
		...
		var imgInfoContent='<li class="col-md-3" data-index="'+i+'">'+
							...
							'<div class="showDetail" data-cont='+JSON.stringify(d)+'>Show Detail<span class="arrow"></span>'+
							'</div></div></div></li>';
	...
	// 获取图片数据show detail
	function getImgData (country) {
		var datepickerVal=$('#datepicker').val().split('-');
		$.ajax({
			type: "GET",
			url: '/analytics/get-account-image-list',
			data: {
	...
	function getImgDataByImgId () {
		...
		$.ajax({
			type: "GET",
			url: '/analytics/search-account-by-imageid',
			data: {
				accountId : $('#account-name').val(),
	...
	function getGeoImgData() {
		...
		var datepickerVal=$('#datepicker').val().split('-');
		$.ajax({
			url: '/analytics/get-account-geo-by-imageid',
			type: 'GET',
			dataType: 'json',
			data: {
				accountId : $('#account-name').val(),
				...
				imageId : $('.append:visible').attr('image-id'),
			},
			success: function(data) {
	...
	function showDetail (obj) {
		...
		var showDetailContent='<tr>'+
							'<td>'+dateVal+'</td>'+
							'<td>'+data.data.mobile_app_install+'</td>'+
							'<td>'+data.data.ecpa+'</td>'+
							'<td>'+data.data.clicks+'</td>'+
							'<td>'+data.data.impressions+'</td>'+
							'<td>'+xs_formate_float(data.data.ctr)+'</td>'+
							'<td>'+xs_formate_float(data.data.cvr)+'</td>'+
							'<td>'+data.data.cpm+'</td>'+
							'<td>'+data.data.spent+'</td></tr>';
		countfl(obj);
		$('.append:visible').attr('image-id', data.id);
	...
	// 内容改变获取数据
	var lastTarget;
	$(document).on('change','#datepicker, #country-name, #sort-result',function() {
		$('#xs_page').val(0);
		$('#xs_before').hide();
