		...
		if(!(autoComplete=$.event['_autoCompleteCache'+setting.srvName])){			//autoComplete主程序,试图在服务名称相同时不再次实例化
			$.event['_autoCompleteCache'+setting.srvName]=autoComplete=(function(){			//保存到全局缓存
				...
				autoComplete.resultPanel={
					isHidden:true,
					$panel:$('<ul id="'+setting.id+'" class="dropdown-menu" style="z-index:9999;"></ul>').hide().appendTo($('body').click(function(){
						autoComplete.resultPanel.hide();
					})).click(function(e){
					...
					_buildList:function(results){
						...
						for (var index = 0; index < results.length; index++) {
							tmp.push('<li resultIndex="'+index+'"><a href="javascript:void(0)">'+results[index].label+'</a></li>');
						};
						return tmp.join('');
					},
					show:function(target){
						if(target){
							...
							this.$panel.css({
								...
								width:(setting.followMine?240:$target.outerWidth())+50
							});
						}
						this.$panel.show();
						this.isHidden=false;
