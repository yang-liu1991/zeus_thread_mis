...
<!-- 以下为列表搜索框 -->
<div class="parment-search">

	<?php
		Pjax::begin(['id' => 'paremnt-search-pjax']);

		$form = ActiveForm::begin([
		'id' => 'parment-search-form',
        'action' => ['pay-list'],
        'method' => 'get',
		'options' => ['class' => 'form-inline well', 'style' => 'text-align:right;'],
		'fieldConfig' => [
	    	'template' => "{beginWrapper}\n<div style='width:300px;'>{input}</div>\n{hint}\n{endWrapper}",
		]
	]); ?>
	...
	<?= $form->field($searchModel, 'search')->widget(Select2::classname(), [
		...
		<?= Html::Button('Export', ['class' => 'btn btn-primary', 'id' => 'parment-export-button']); ?>
	<?php ActiveForm::end(); ?>
	<?php Pjax::end(); ?>
</div>
